...
class Unet_const:
    ...
    ВНИМАНИЕ!  Если возник конфликт имен слоев активационных функций, то перезапустите
    создание сети вновь и уже конфликта не будет
    ...
    enc_pool - нужно указать список из'Maxpool' для MaxPooling2D и 'Avrpool' для AvgPool2D
               в енкодере. То есть можно указать тип пулинга каждому уровню, причем
               если количество уровней больше, чем указано, то далее берется
               значение по индесу -1
    enc_qtyconv - список из одного или нескольких int, понимается как кколичество
                  сверточных блоков на каждом уровне энкодера, причем  если
                  количество уровней больше, чем указано, то далее берется
                  значение по индесу -1
    enc_kernels - список из кортежей фильтров применяемых к входному массиву
                  сверточных блоков на соответствующем уровне энкодера, причем
                  если количество уровней  больше, чем указано, то далее берется
                  значение по индесу -1
    enc_activ - список из строчных имен типов активационных функций, применяемых
    ...
    enc_usebtchnorm - список из False или True, использывать или нет BatchNormalization
    ...
    btlnec_usebtchnorm - спользывать или нет BatchNormalization в сверточном блоке
                         в botelnec
    ----- decoder --------------------------------------------------------------
    dec_uplayer - нужно указать список из 'Conv2DT' для Conv2DTranspose и 'UpSampl'
    ...
    dec_qtyconv - список из одного или нескольких int, понимается как кколичество
                  сверточных блоков на каждом уровне декодера, причем  если
                  количество уровней больше, чем указано, то далее берется
                  значение по индесу -1
    dec_kernels - список из кортежей фильтров применяемых к входному массиву
                  сверточных блоков на соответствующем уровне декодера, причем
                  если количество уровней больше, чем указано, то далее берется
                  значение по индесу -1
    dec_activ - список из строчных имен типов активационных функций, применяемых
                на соответствующем уровне декодера, , причем  если количество уровней
                больше, чем указано, то далее берется  значение по индесу -1
    dec_usebtchnorm - список из False или True, использывать или нет BatchNormalization
    ...
    def __init__(self,
        ...
        self.levels = levels
        self.residual = [residual] if type(residual) == bool else residual
        self.initializer = kernel_initializer
        self.regularizer = kernel_regularizer
        self.pretatrain_net = pretatrain_net
        self.trainable = train_pretrain_lays
        self.dropout = [(False, False)] if type(enc_dropout) == bool else enc_dropout
        self.enc_pool = [enc_pool] if type(enc_pool) == str else enc_pool
        self.enc_qtyconv = [enc_qtyconv] if type(enc_qtyconv) == int else enc_qtyconv
        self.enc_kernels = [enc_kernels] if type(enc_kernels[0]) == int else enc_kernels
        self.enc_activ = [enc_activ] if type(enc_activ) == str else enc_activ
        self.enc_usebtchnorm = [enc_usebtchnorm] if type(enc_usebtchnorm) == bool else enc_usebtchnorm
        ...
        self.dec_qtyconv = [dec_qtyconv] if type(dec_qtyconv) == int else dec_qtyconv
        self.dec_kernels = [dec_kernels] if type(dec_kernels[0]) == int else dec_kernels
        self.dec_activ = [dec_activ] if type(dec_activ) == str else dec_activ
        self.dec_usebtchnorm = [dec_usebtchnorm] if type(dec_usebtchnorm) == bool else dec_usebtchnorm
        self.dec_uplayer = [dec_uplayer] if type(dec_uplayer) == str else dec_uplayer
